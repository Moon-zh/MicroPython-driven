...
class PCD8544:
	...
	def __init__(self, spi, rst, ce, dc, light, pwr=None):
		...
		self.lcd_pictuer = pictuer.pictuer()
		self.hc=[[0 for i in range(84)] for j in range(6)]
		self.tab1=[0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80]
	...
	def lcd_write_draw(self, x, y, a):
		self.position(x, y)
		self.data([a])

	def lcd_write_draw_x(self, xq, yq, xz,yz):	
		if xz>xq:
			bx=1
			maxx=xz-xq
		else :
		...
		while True:
			a=(self.tab1[int(xq%8)])|(self.hc[int(xq/8)][yq])
			self.lcd_write_hc(int(xq/8),yq,a)
			print(yq,int(xq/8),a)
			if (xq==xz) and (yq==yz) :
				print('stop')
				break
			print(max,maxx,maxy,xq,yq,xz,yz,bx,by)
			bzx+=maxx
			bzy+=maxy
			if bzx>max :
				bzx-=max
	...
	def lcd_write_pictuer(self, x, y,a,b):
		self.position(x, y)
		print('pictuer')
		for i in range(a,b):
			self.data(self.lcd_pictuer.get_pictuer(i))
	
	def lcd_write_hc(self, x, y,a):
		self.hc[x][y]=a
			
	def lcd_read_hc(self):
		self.position(0, 0)
		for i in range(6):
			self.data(self.get_hc(i))
					
	def get_hc(self, data):
		return self.hc[data]